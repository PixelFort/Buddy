    ...
    <>
      <SafeAreaView>
        <MapView
          ...
          {circularPoints?.map((point, index) => {
            const user = user[index % user.length]; //cycle  through the user if there are more points
            return (
        ...
        <View
          ...
          <Image
            style={{width: 110, heigth: 60, resizeMode: 'contain'}}
            source={{
      ...
      <View style={{padding: 10, backgroundColor: 'white', margintop: 'auto'}}>
        <Pressable
          ...
          <Text style={{textAlign:"center",color:"white",fontWeight:"500"}}>READY SET Goal</Text>
        </Pressable>
      </View>
    </>
  );
